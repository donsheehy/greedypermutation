...
class VizFVMNeighborGraph(Group):
    """
    A utility class used to draw visualizations of FVMNeighborGraph objects as
    a series of points, edges, as well as convex hulls.

    Attributes
    ----------
    neighbor_graph: FVMNeighborGraph
      a FVMNeighborGraph object to draw to a canvas.
    style: str
      The object style used to draw a FVMNeighborGraph as an str. (default 'neighbor_graph')
    stylesheet: StyleSheet
      The stylesheet used to describe which elements to draw an how to draw them. (default ds2viz.default_styles)
    """

    def __init__(
        self, neighbor_graph, style="neighbor_graph", stylesheet=default_styles
    ):
        """
        Parameters
        ----------
    ...
    def _cell_edges(self):
        ...
        if edge_style is not None:
            for cell in self.N.vertices():
                for point in cell.points:
                    self.addelement(
                        Line(point.center, cell.center, edge_style, self.stylesheet)
                    )
    ...
    def _vertices(self):
        ...
        if vertex_style is not None:
            ...
            vertex_radius = (
                2
                if vertex_style_dict.get("radius") is None
                else vertex_style_dict["radius"]
            )
        ...
        if edge_style is not None:
            for e in self.N.edges():
                if len(e) == 2:
                    u, v = e
                    self.addelement(
                        Line(u.center, v.center, edge_style, self.stylesheet)
                    )
    ...
    def _hull(self):
        ...
        if hull_style is not None:
            for v in self.N.vertices():
                self.addelement(
                    Polygon(
                        convexhull(p.center for p in v.points),
                        hull_style,
                        self.stylesheet,
                    )
                )
